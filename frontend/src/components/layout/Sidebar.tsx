...
export function Sidebar() {
  ...
  return (
    <motion.aside
      initial={{ x: -250 }}
      animate={{ x: 0 }}
      className="fixed inset-y-0 left-0 z-50 w-64 border-r border-white/10 bg-background/50 backdrop-blur-xl flex flex-col"
    >
      ...
      <div className="p-4 border-t border-white/10">
        <div className="flex items-center gap-3 rounded-lg px-3 py-3 bg-white/5 border border-white/5">
          <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-lg">
            TS
          </div>
          <div className="flex flex-col flex-1">
            ...
            <span className="text-xs text-zinc-500">Owner / Admin</span>
          </div>
          <button onClick={handleLogout} className="p-1.5 rounded-md hover:bg-white/10 text-zinc-400 hover:text-red-400 transition-colors" title="Logout">
